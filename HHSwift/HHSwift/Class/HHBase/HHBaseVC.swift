...
class HHBaseVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        let navBgImage = UIImage.imageWithColor(color: UIColor.white, size: CGSize.init(width: kWidth, height: kNavBarHeight))
        let  navBgImage1 = navBgImage?.tintedImage(color: UIColor.red, rect: CGRect.init(x: 0, y: 0, width: kWidth, height: kNavBarHeight), alpha: 0)
